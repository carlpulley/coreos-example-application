...
trait ProfileService extends Directives with CommonMarshallers with CommonPathDirectives {
  ...
  def boot(address: Address, userProfile: ActorRef, userProfileProcessor: ActorRef) = RestApi(
    route = Some({ ec: ExecutionContext => userProfileRoute(userProfile, userProfileProcessor)(ec) })
  )

  private def userProfileRoute(userProfile: ActorRef, userProfileProcessor: ActorRef)(implicit ec: ExecutionContext) =
    path("user") {
      post {
